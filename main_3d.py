    ...
    for fname in tqdm(image_list[:5], desc="Processing"):
        if fname.lower().endswith((".jpg", ".jpeg", ".png")):
            ...
            if dataset == "area3":
                ...
                depth_path = os.path.join(depth_dir, fname.replace('rgb.png', 'depth.png'))
                semantic_labels = load_labels(semantic_path)
                if os.path.exists(image_path):
                    data_num = process_area3(image_path, annot_path, xyz_path, depth_path, semantic_labels, MODEL_NAME, client, output_path)
                    data_nums += data_num
